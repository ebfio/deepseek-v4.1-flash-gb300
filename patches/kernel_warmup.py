...
def _run_flashinfer_autotune_dummy_runs(
    ...
    for num_tokens in _flashinfer_autotune_token_counts(runner, include_bf16=False):
        ...
        # round_up=True: map runtime M to the smallest bucket >= M, matching
        # what mm_mxfp8's own mapper (map_to_hybrid_bucket_uncapped) does at
        # serving time. The autotuner's override default is floor mapping,
        # which hands a cute-dsl low-M split-K tactic tuned at bucket b to
        # calls with b < M < next bucket; the runner then rejects the tactic
        # (its MMA tile is chosen by rounding M up) and the dummy run raises
