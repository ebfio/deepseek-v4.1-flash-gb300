...
def _dgl_log(tag, *parts):
    """One log line per distinct (tag, parts) combination."""
    key = (tag, parts)
    if key in _DGL_LOG_SEEN:
        return
    _DGL_LOG_SEEN.add(key)
    import logging
    logging.getLogger("dsv41.scales").error("SCALES %s %s", tag, " ".join(str(p) for p in parts))


def _dg_grouped_mn_major(sf: torch.Tensor) -> torch.Tensor:
    """Reinterpret a grouped int32 UE8M0 scale tensor as DeepGEMM's grouped
    MN-major TMA-aligned layout.

    DeepGEMM validates (csrc/utils/layout.hpp::check_sf_layout):
        stride(-2) == 1
        stride(-1) == get_tma_aligned_size(mn, element_size)
        stride(-3) == stride(-1) * size(-1)

    The loader emits row-major (G, mn, kb). This rebuilds the same values with
    physical [G][kb][align4(mn)] and returns the [G][mn][kb] view, so mn is
    padded to a multiple of 4 rather than assumed aligned. Value-preserving:
    result[i, j, k] == sf[i, j, k]; only strides and padding change.
    """
    G, mn, kb = sf.shape
    mn_pad = (mn + 3) // 4 * 4
    buf = torch.zeros((G, kb, mn_pad), dtype=sf.dtype, device=sf.device)
    buf[:, :, :mn].copy_(sf.transpose(-1, -2))
    return buf.transpose(-1, -2)[:, :mn, :]
...
def deepgemm_post_process_weight_scale_block(
    ...
    # SM103/DeepGEMM: transform_sf_into_required_layout hands back a row-major
    # grouped tensor whose stride(-2) != 1, which the grouped-kernel layout
    # checks reject. Rebuild it grouped MN-major. Applies to both the attention
    # o_proj weight scales and the MoE expert scales, since both come through
    # here. 2-D (non-grouped) tensors are already handled upstream.
    if out.dtype == torch.int32 and out.dim() == 3 and out.stride(-2) != 1:
        out = _dg_grouped_mn_major(out)
        _dgl_log("relayout", tuple(out.shape), out.stride())
    return out
