...
def sparse_attn_indexer(
    ...
    if has_decode:
        ...
        # SM103 (GB300) has no cooperative_topk cubin in _C_stable_libtorch:
        # its TORCH_CUDA_ARCH_LIST stops at 10.0+PTX, so the launch dies with
        # "no kernel image is available for execution on the device" during graph
        # capture. has_device_capability(90) admits SM103; persistent_topk below
        # covers the same conditions and works. Restrict to the SM90 family.
        _is_sm90_family = current_platform.is_device_capability_family(90)
