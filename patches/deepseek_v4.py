...
# </|...> follows the value is its closer, well-formed or not (</|> is the
# common production shape); running on to the next well-formed closer instead
# swallows every parameter in between. A <|... opener closes the value
# implicitly and is left for the next match.  (upstream #56302)
# The string="..." attribute is OPTIONAL: the degraded dialects omit it
# entirely. is_str is then None and the converter treats the value as a string,
# which the engine's schema-aware coercion re-types for non-string params.
...
def deepseek_v4_config(thinking: bool = False) -> ParserEngineConfig:
    return ParserEngineConfig(
        ...
        transitions={
            ...
            # Orphan-invoke recovery: the model sometimes omits the
            # <｜DSML｜tool_calls> START wrapper at long context (vLLM #48931 /
            # #51914). Enter the tool path directly on the invoke prefix so a
            # complete invoke block still becomes a tool call instead of
            # leaking as assistant content.
